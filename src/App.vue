...
import type { Furniture, MaterialType } from "./types";
...
const showFurniturePanel = ref(false);
...
function handleClear() {
  ...
  showFurniturePanel.value = false;
  selectedFurniture.value = null;
}

function handleToggleFurniture() {
  showFurniturePanel.value = editorStore.mode === "furniture";
  if (!showFurniturePanel.value) {
    selectedFurniture.value = null;
    if (threeSceneRef.value) {
      threeSceneRef.value.deselectFurniture();
    }
  }
}
...
function handleChangeMaterial(materialType: MaterialType) {
  if (threeSceneRef.value) {
    threeSceneRef.value.changeFurnitureMaterial(materialType);
  }
}

function handleDeleteFurniture() {
  if (threeSceneRef.value) {
    threeSceneRef.value.deleteSelectedFurniture();
    selectedFurniture.value = null;
  }
}
...
watch(
  ...
  (message) => {
    if (message.includes("已创建") || message.includes("已生成") || message.includes("已添加")) {
