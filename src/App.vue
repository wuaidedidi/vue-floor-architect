<template>
  <div class="app-container">
    <Toolbar
      :is-f-p-mode-active="isFPModeActive"
      @upload="handleUpload"
      @load-default="handleLoadDefault"
      @generate="handleGenerate"
      @clear="handleClear"
      @toggle-fp-mode="handleToggleFPMode" />
...
const isFPModeActive = ref(false);
...
function handleToggleFPMode() {
  if (threeSceneRef.value) {
    threeSceneRef.value.toggleFirstPersonMode();
    isFPModeActive.value = threeSceneRef.value.isFPModeActive;

    if (isFPModeActive.value) {
      displayToast("已进入第一人称漫游模式，按 ESC 退出", "info");
    } else {
