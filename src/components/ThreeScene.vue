...
const nightMode = useNightMode(sceneRef, cameraRef, controlsRef);
...
function initThree() {
  ...
  // 设置夜景模式的环境光引用
  nightMode.setAmbientLight(ambientLight);
...
defineExpose({
  ...
  uploadFloorPlan,
  toggleNightMode: () => {
    const isEnabled = nightMode.toggleNightMode();
    editorStore.setNightMode(isEnabled);
    return isEnabled;
  },
  enableNightMode: () => {
    nightMode.enableNightMode();
    editorStore.setNightMode(true);
  },
  disableNightMode: () => {
    nightMode.disableNightMode();
    editorStore.setNightMode(false);
  },
  isNightMode: () => nightMode.isNightMode.value,
});
