...
import type { Point3D, FloorPlanImage, FurnitureType } from "../types";
...
import { createFurnitureByType } from "../utils/furnitureModels";
 ...
 * 检测点击的家具
 */
function getClickedFurniture(event: MouseEvent): THREE.Mesh | null {
  if (!canvasRef.value) return null;
  ...
  scene.traverse((obj) => {
    ...
    // 也检查分组中的对象
    if (obj instanceof THREE.Group && obj.name.startsWith("furniture-")) {
      obj.traverse((child) => {
 ...
 * 在场景中添加家具
 */
function addFurnitureToScene(type: FurnitureType, position: THREE.Vector3) {
  const furniture = createFurnitureByType(type, position);
  if (!furniture) return;

  // 设置家具名称标识
  furniture.mesh.name = `furniture-${furniture.id}`;

  // 遍历设置所有子对象的名称（用于射线检测）
  furniture.mesh.traverse((child) => {
    if (child instanceof THREE.Mesh) {
      child.userData.furnitureId = furniture.id;
    }
  });
  ...
  console.log(`已添加家具: ${furniture.name}`, furniture);
  editorStore.updateStatusMessage(`已添加${furniture.name}`);
}
...
function addTestFurniture() {
  // 添加沙发
  addFurnitureToScene("sofa", new THREE.Vector3(-25, 0, 0));
  // 添加柜子
  addFurnitureToScene("cabinet", new THREE.Vector3(25, 0, 0));
  // 添加桌子
  addFurnitureToScene("table", new THREE.Vector3(0, 0, 25));
  // 添加椅子
  addFurnitureToScene("chair", new THREE.Vector3(-10, 0, 25));
  addFurnitureToScene("chair", new THREE.Vector3(10, 0, 25));

  editorStore.updateStatusMessage("测试家具已添加，点击家具进行换装！");
}

/**
 * 处理家具选择
 */
async function handleFurnitureSelection(event: MouseEvent) {
  const clickedMesh = getClickedFurniture(event);

  if (clickedMesh) {
    // 找到对应的家具对象
    const furnitureId = clickedMesh.userData.furnitureId;
    const furniture = useFurnitureDressing().furnitureObjects.value.find((f) => f.id === furnitureId);

    if (furniture) {
      await selectFurniture(furniture, scene);
      editorStore.updateStatusMessage(`已选中: ${furniture.name}，使用右侧面板更换材质`);
    }
  } else {
    // 点击空白处取消选择
    deselectFurniture(scene);
    editorStore.updateStatusMessage("已取消选择");
  }
}
...
function handleClick(event: MouseEvent) {
  ...
  } else if (editorStore.mode === "select" || editorStore.mode === "none") {
    // 在选择模式或默认模式下可以选择家具
    handleFurnitureSelection(event);
  }
}
