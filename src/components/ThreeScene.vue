<template>
  <div class="three-scene-container" ref="containerRef">
    ...
    <!-- 绘制辅助提示 -->
    <div class="drawing-hints" v-if="showHints && !isFPModeActive">
    ...
    <!-- 第一人称模式提示 -->
    <div class="fp-mode-hint" v-if="isFPModeActive">
      <div class="fp-hint-title">🎮 第一人称漫游模式</div>
      <div class="fp-hint-controls">
        <div class="fp-control-item"><kbd>W/A/S/D</kbd> 移动</div>
        <div class="fp-control-item"><kbd>Shift</kbd> 加速</div>
        <div class="fp-control-item"><kbd>鼠标</kbd> 视角</div>
        <div class="fp-control-item"><kbd>ESC</kbd> 退出</div>
      </div>
    </div>

    <!-- 快捷键提示 -->
    <div class="shortcuts-hint" v-if="!isFPModeActive">
...
import { createRoomLighting } from "../composables/useVolumetricLight";
import { createAutoDoors, generateRoomDoors } from "../composables/useAutoDoor";
...
let roomLighting: ReturnType<typeof createRoomLighting> | null = null;
let autoDoors: ReturnType<typeof createAutoDoors> | null = null;
let isFPModeActive = ref(false);
...
function animate() {
  ...
  if (isFPModeActive.value && fpControls) {
    fpControls.update();
...
function initFirstPersonMode() {
  if (!canvasRef.value || fpControls) return;
  ...
  // 创建第一人称控制器
  fpControls = useFirstPersonControls(camera, canvasRef.value, scene);
}
