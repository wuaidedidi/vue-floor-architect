...
export const useEditorStore = defineStore("editor", () => {
  ...
  const constructionPhase = ref<ConstructionPhase>("rough");
  const constructionProgress = ref(0); // 当前阶段内的进度 0-1
  ...
  function setConstructionPhase(phase: ConstructionPhase, progress: number = 0) {
    constructionPhase.value = phase;
    constructionProgress.value = Math.max(0, Math.min(1, progress));
  }
